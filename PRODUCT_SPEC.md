...
**Current (v0.6):** Standalone HTML pages served at `/artifact/{type}/{house_id}`:
...
**Planned (v0.8) — Visual Artifact Engine:** Three rendering paths, each targeting a different artifact class:

**Path A — Fabric.js (visual & graphic artifacts)**
Canvas-based browser rendering for one-pagers, battlecards, social cards, event briefs. The LLM generates a **design JSON spec** describing layout zones, text blocks, image placeholders, and brand color tokens. The browser deserializes this into a Fabric.js canvas. Key properties:
- Logo and image insertion via `fabric.Image.fromURL()` — no server round-trip; accepts URL, file upload, or base64
- Per-artifact-type templates with defined content zones and brand styling
- Interactive before export — user can drag/replace images, edit text in-canvas
- Export: PNG (`canvas.toDataURL()`), PDF (jsPDF), SVG for design hand-off
- New MCP tool: `generate_fabric_artifact` returns the design JSON spec grounded in the full message house

**Path B — reveal.js (presentations & slide decks)**
HTML-based presentation engine for sales decks, event presentations, executive readouts. The LLM generates structured slide JSON; the server renders it to reveal.js HTML via Jinja2. Key properties:
- New skill types: `sales_deck`, `event_presentation`, `executive_readout`
- Custom CSS theme per workspace — brand colors, fonts, logo applied at render time
- Designated image/logo zones per slide template
- Speaker notes generated by LLM from grounding context
- PDF export via reveal.js `?print-pdf` mode + browser print engine (higher quality than jsPDF)
- New MCP tool: `build_presentation` returns a live reveal.js presentation URL

**Path C — Penpot (highest-fidelity design export)**
For artifacts where pixel-perfect design quality matters most. MsgStack creates a fully designed document in Penpot via API — brand fonts, vector assets, image frames, design tokens — and returns an edit link. MsgStack already has the Penpot MCP server connected.
- Auto-creates a Penpot project per MsgStack workspace; syncs brand color tokens and fonts
- Programmatically builds artifact pages (frames, text layers, image placeholders, brand colors)
- New MCP tool: `export_to_penpot` creates the artifact and returns a direct edit link
- User lands in Penpot for final polish and export — SVG, PNG, or PDF at full design fidelity

**Renderer routing:** Each skill's metadata includes a `renderer` field (`html`, `fabric`, `reveal`, `penpot`). `generate_artifact` routes to the appropriate renderer. A shared `ArtifactRenderer` abstraction keeps renderer additions isolated from core generation logic.
...
| Visual artifacts — graphic/canvas | Fabric.js (planned v0.8) |
...
The next major milestone replaces server-rendered HTML artifact pages with a three-path browser-side design engine supporting real graphic design, logo/image insertion, and export to PNG/PDF. See [ROADMAP.md](ROADMAP.md) for full feature breakdown.

- **Fabric.js (visual artifacts):** Canvas-based rendering from LLM-generated design JSON; logo/image insertion; per-artifact-type templates; PNG/PDF/SVG export; interactive before export
- **reveal.js (presentations):** Slide deck generation for sales decks, event presentations, executive readouts; workspace-branded themes; speaker notes; high-quality PDF export via browser print engine
- **Penpot (design export):** Programmatic artifact creation via Penpot API (already connected via MCP); brand token sync; returns edit link for final polish at full design fidelity
- **ArtifactRenderer abstraction:** Renderer field on each skill routes `generate_artifact` to the correct path; new renderers added without touching core generation

### v0.9 — Document Source Integrations

Connects MsgStack to where marketing documents already live — eliminating manual upload and keeping frameworks in sync as source documents evolve.
